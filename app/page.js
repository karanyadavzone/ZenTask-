...
export default function HomePage() {
  ...
  const handleSwipeRight = async (task) => {
    // Swipe right = complete task
    await handleQuickComplete(task)
  }

  const handleSwipeLeft = async (task) => {
    // Swipe left = delete task (move to trash)
    try {
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    return activeFilters.every(filter => {
      switch (filter.type) {
        ...
        case 'tag':
          // Handle both database format (task_tags) and sample format (tags array)
          if (task.task_tags) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        {tasksLoading ? (
          ...
          >
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {searchQuery.trim() ? 'No tasks match your search' : 'No tasks found'}
            </h3>
      ...
      <button
        onClick={() => setShowCreateTask(true)}
        className="fixed bottom-20 right-6 w-12 h-12 bg-purple-600 text-white rounded-full shadow-lg flex items-center justify-center hover:bg-purple-700 active:scale-90 transition-all z-20"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
